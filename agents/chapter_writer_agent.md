...
## 卡片到正文的转写铁律（四条，沿用 stage-7-writing.md §7.0）

- **禁止**将卡片字段逐条翻译为段落（案例卡的"时间线/背景/动作/效果"不是 4 个自然段）。
- **禁止**在正文出现字段标签（"时间线：2023年..."、"效果：增长240%"）。
- **每条关键判断呈现完整三角**：主张 → 证据 → 推理。卡片提供"证据"角，你补全"主张"和"推理"。⚠ "证据"角是可追溯的事实来源（机构+日期+数据），**不是**证据的等级评价或核验取舍过程。
- **禁止把后台过程当正文**：证据等级评价（A/B/C 级）、来源取舍（采用 X 不采用 Y）、核验状态自述、claim_id、方法论自评均属"后台"（见 `references/writing-standards.md` 标准 0），不得作为正文叙事对象——它们下沉脚注/来源标注，不进叙事主体。判定测试：删掉这句话，读者对"结论是什么"的理解是否受损？不受损即后台。
