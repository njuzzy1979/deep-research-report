...
|------|---------|------|----------------------------------------|
| **R1** | 全文不写 H1、不写 H2；章首 blockquote 必须逐字以 `> **本章结论**：` 开头 | C1/C2、F5 | writer-template §2.1、职责边界 |
...
| **R5** | 正文落盘后，必须将自声明条目追加写入 `research/claims/各章节自声明.md` | `contract_check.py` C8（FATAL） | writer-template §6、输出契约 |
...
1. **当前章草稿** `research/drafts/chXX-<描述>.md`——**分章文件以 `> **本章结论**：...` blockquote 起始**（红线 **R1**，严格遵循 `references/writer-template.md` 定义的强制输出骨架），随后以分隔线 `---` 过渡，正文节用 H3/H4（`### 节标题` / `#### 小节标题`，只写纯文字不加编号，红线 **R2**）。分章文件中**不使用 H1 和 H2**——H1 和 H2（章容器）在合并终稿中由 `finalizer_agent` 统一管理。遵守转换器合约 C1-C9。**标题文本只从 YAML `section_title` 字段取纯文字**。
...
1. `## 草稿正文`——当前章完整正文，H2 起始不加编号，遵守大纲论证路径、卡片转写铁律、写作标准、合约 C1-C5。每个关键判断带来源引用。**不含写作者自声明**——自声明条目通过 Write/Edit 工具追加到 `research/claims/各章节自声明.md`。
