...
| 26 | **画布内直接跨图引用**——节点文本内容中直接写入其他图的图号，如"六维结构历史观测序列（图3-1）""六类关系图谱当前状态（图3-2）"（真实项目 3-3 图 `in1`/`in2` 实测命中），读者必须翻到另一张图才能理解本图这个节点在说什么，增加不必要的跨图阅读负担 | 节点文本应是不依赖图号的自足描述——直接概括被引用图的内容要点（如把"图3-1"替换为其内容本体的简短复述），而不是仅给出一个图号让读者自行去查；确有必要建立图间关联时，那是正文 Markdown 层面的"参见图X-X"叙述职责，不应内嵌进画布；出图后应跑 `drawio_layout_validator.py` 的 G12 判据（`CROSS_FIGURE_REFERENCE`）核实无跨图引用命中（本图自身图号的自我标注不算跨图引用，由 G6 图注判据单独处理，不与 G12 重复计） |
