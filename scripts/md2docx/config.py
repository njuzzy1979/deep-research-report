...
FRONT_BACK_WORDS = ("摘要", "执行摘要", "目录", "图表目录", "索引", "术语表",
                      "前言", "导论", "绪论", "引言")
# 出处：V3.0 §一 文档结构表（结构件名）；02-algorithms.md §C.3 FRONT_BACK_WORDS
# V2.1 扩展：增加"前言/导论/绪论/引言"以支持前言章的正确识别
# "参考文献"已移出本白名单——它不属于"前置件"（渲染为 Heading 2 的 ABSTRACT
# 分类），而是与正文/附录并列的独立报告级组成部分（研究报告格式规范.md
# "一、文档结构"序号6），须渲染为 Heading 1，见下方 N_08_REFERENCES 与
# HeadingKind.REFERENCES（ir.py）。此前二者共用同一白名单/分类，导致
# 参考文献被错误降级为附录内所属章的子节。
...
# ---- 02 §A 图三元组解析 ----
# Phase 6.3：数字捕获组改为可选——图号不再从作者手写文本"誊抄"，而是由
# assemble 层依据文档结构（所在章 + 章内出现顺序）计算得出。数字组保留
# 仅用于向后兼容旧文档格式，以及与结构计算值做交叉核对（不一致时告警）。
# 捕获组序号随之改变：group(1)/group(2) 为可选的旧式数字（可能为 None），
# 题注文字固定为 group(3)。
RE_IMG_LINE = r"^\s*!\[([^\]]*)\]\((.+)\)\s*$"
RE_FIG_ALT = r"^图(?:(\d{1,2})-(\d{1,2}))?(?:[ 　]+|[：:][ 　]*)(.+)$"

# ---- 02 §B 表题注解析 ----
# 同上：数字组可选，caption 文字固定为 group(3)。
...
N_01_CHAPTER_CN = r"^第\s*([一二三四五六七八九十百零两〇]{1,4})\s*(?:章|部分|篇|编)\s*[：:、．.]?\s*"
N_02_CHAPTER_ARABIC = r"^第\s*(\d{1,3})\s*(?:章|部分|篇|编)\s*[：:、．.]?\s*"
N_03_SUBSECTION = (
    r"^(\d{1,2})[.．](\d{1,2})[.．](\d{1,2})[.、]?(?:[ 　]+|(?=[一-鿿（【《“「]))"
)
N_04_SECTION = r"^(\d{1,2})[.．](\d{1,2})[.、]?(?:[ 　]+|(?=[一-鿿（【《“「]))"
N_05_CHAPTER_DUNHAO = r"^(\d{1,3})[.、．]\s*"
N_06_CHAPTER_CN_DUNHAO = r"^([一二三四五六七八九十〇]{1,3})[、．.]\s*"
N_07_APPENDIX = r"^附录\s*([A-Za-z]|[一二三四五六七八九十]{1,3})?\s*[：:、．.]?\s*"
# 精确匹配"参考文献"标题（允许尾随全/半角冒号），用于 HeadingKind.REFERENCES
# 分类判定——与 N_07_APPENDIX 同类地位，都是"报告级组成部分"标题识别正则，
# 不放进 FRONT_BACK_WORDS（那是"前置件"白名单，语义不同）。
N_08_REFERENCES = r"^参考文献\s*[：:]?\s*$"
