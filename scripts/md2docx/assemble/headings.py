...
if __name__ == "__main__":
    ...
    # --- 场景4：「## 摘要」 → CHAPTER（无显式编号，fall-through 默认章）---
    print("\n=== 场景4：摘要 H2 ===")
    c4 = IssueCollector()
    tokens4 = [HeadingToken(level=2, raw_text="摘要", source_line=10)]
    r4 = classify_and_number(tokens4, c4)
    check("kind=CHAPTER", r4[0].kind == HeadingKind.CHAPTER, str(r4[0].kind))
    check("display_number='第一章'", r4[0].display_number == "第一章", r4[0].display_number)
    check("number=1", r4[0].number == 1, str(r4[0].number))
    ...
    # --- 场景9："执行摘要" H2 → CHAPTER（无显式编号，fall-through 默认章）---
    print("\n=== 场景9：执行摘要 ===")
    c9 = IssueCollector()
    tokens9 = [HeadingToken(level=2, raw_text="执行摘要", source_line=10)]
    r9 = classify_and_number(tokens9, c9)
    check("kind=CHAPTER", r9[0].kind == HeadingKind.CHAPTER, str(r9[0].kind))
