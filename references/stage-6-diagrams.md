...
**陷阱 1：容器/分组框的 XML 声明顺序。** drawio XML 按声明顺序绘制，后声明的元素遮盖先声明的。白色或非透明填充的分组框（container/group）若在子节点**之后**写入 XML，会完全遮盖子节点——导出 PNG 上分组框内部一片空白，只剩连接箭头穿过。机器门禁 G1-G12 均不检查绘制顺序（纯几何检查），只有导出 PNG 人工确认才能发现。标准做法：先用纯计算函数（`text_size`/`_uniform_row_font_and_box`）算子节点尺寸，再调用 `add_container()`，最后实际放置子节点。

**陷阱 2：导出操作可能隐式写回/污染源 `.drawio` 文件。** draw.io 桌面版在执行 `-x -f png/svg` 导出时，Electron 应用可能以内部格式自动保存源文件（属性顺序重排、`<mxfile host="Electron">` 包裹等），导致后续布局门禁误报或文件格式异常。推荐流程：先完成全部设计迭代（此阶段只跑门禁不导出）→ 门禁全绿后统一做一轮 PNG 导出 → 若发现视觉问题，回生成脚本改（不改 `.drawio`），改完重新生成并重跑门禁。若导出后仍需跑门禁，先确认源文件字节内容与生成脚本最近一次输出一致。
