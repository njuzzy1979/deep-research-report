...
**为什么阶段 9 也要跑一次**：hook 分发挂在阶段 1.2，但若本次会话是"**半路接手**"——跳过阶段 1、直接从已有草稿进入阶段 7-9（**这正是本次事故的实际形态之一**）——hook 从未被下发，两条护栏规则在该会话中完全不生效。本脚本幂等（已安装则跳过、不重复追加 hook 条目），故无条件先跑一次即可闭掉这个下发盲区。

> 本阶段对 hook 的依赖是**加强层**而非前置条件：即使 hook 未安装，`.partial` 转正机制与 `verify_docx` 回读校验仍然生效——它们是产物层面的机器门禁，不依赖行为层面的拦截。
