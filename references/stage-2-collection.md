...
**验证规则（硬门槛）**：
- 统计"应下载"数 = source-index.csv 中 `source_type` 为 PDF/网页且非"在线资源"的条目数。
- 统计"实际已下载"数 = `research/sources/` 中实际存在的 PDF/MD 文件数。
- **若实际已下载数 < 应下载数的 80% → STOP，回炉补充下载**，不得直接进入 2.3 抽取。
- **不允许出现 source-index.csv 中 80% 以上条目 `local_path` 为空的情况**——若出现，说明下载步骤被跳过，回到 §2.0"下载保存"重做。范围 A（仅用户素材）不适用本门槛的"扩展搜集下载"部分，但用户素材本身仍须在 sources/ 中可见。
