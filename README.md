...
| 工具 | 用途 | 安装方式 | 配置说明 |
|------|------|---------|---------|
| **web-search-skill** | 通用网页搜索（Tavily + 百度双引擎），支持关键词搜索、URL 提取、整站爬取、深度研究报告 | 将 skill 克隆到 Claude Code 技能目录：`git clone <repo-url> web-search-skill` | 设置环境变量 `TAVILY_API_KEY` 和 `QIANFAN_API_KEY`。脚本位于 `web-search-skill/scripts/search.js` |
...
| **MinerU** | 文档精准解析（PDF/Office/图片/HTML → Markdown），含 OCR 扫描件识别、表格还原、公式提取 | 将 skill 克隆到 Claude Code 技能目录 | 设置环境变量 `MINERU_TOKEN`（在 https://mineru.net/apiManage 申请）。脚本位于 `mineru/scripts/mineru_parse.py` |

### 出图工具

| 工具 | 用途 | 安装方式 | 配置说明 |
|------|------|---------|---------|
| **draw.io 桌面版** | 架构图、流程图绘制，导出 `.drawio` + `.svg` | 从 [draw.io 官网](https://github.com/jgraph/drawio-desktop/releases) 下载安装 | Windows 默认安装路径 `D:\Program Files\draw.io\`（或其他盘符）。安装后可通过 `where draw.io`（Win）或 `which draw.io`（Linux/macOS）查找路径 |
...
> **提示**：如果你的外部 skill 安装在非标准位置，可将路径添加到 Claude Code 的技能搜索路径中（参见 Claude Code 文档的 "Custom Skills" 部分）。
